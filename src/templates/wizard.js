...
function updateTools(tools, error) {
    ...
    if (toolsList) {
        ...
        tools.forEach((tool, index) => {
            ...
            const buttonBg = isExecuted ? '#28a745' : 'var(--vscode-button-background)';
            const buttonShadow = isExecuted ? '0 2px 4px rgba(40, 167, 69, 0.25)' : '0 2px 6px rgba(0, 0, 0, 0.1)';
            const buttonColor = isExecuted ? 'white' : 'var(--vscode-button-foreground)';
            ...
            if (hasParams && hasDefaultsForRequired) {
                ...
                headerAction = `
                    <div class="tool-header-actions">
                        ...
                        <button data-tool-index="${index}" onclick="runTool(${index}); event.stopPropagation();" style="background: ${buttonBg}; padding: 6px 16px; font-size: 12px; border-radius: 4px; box-shadow: ${buttonShadow}; transition: all 0.2s ease; display: flex; align-items: center; gap: 4px; border: none; color: ${buttonColor}; cursor: pointer;"><svg width="12" height="13" viewBox="0 0 71.884262 76.735161" style="flex-shrink: 0;"><path style="fill:none;stroke:currentColor;stroke-width:10;stroke-linecap:square;stroke-linejoin:miter" d="m 12.259,2 c -4.05249,0.15214 -7.259192,3.48167 -7.258988,7.53701 v 25.65631 0.39946 25.65632 c -1.58e-4,5.79375 6.261243,9.42401 11.289233,6.54533 l 45.286075,-23.39342 a 10.1794,10.1794 89.96398 0 0 -0.0114,-18.09387 L 16.28924,2.99196 c -1.224231,-0.70098 -2.620523,-1.04455 -4.030245,-0.99167 z" /></svg>Run</button>
                    </div>`;
            ...
            html += `
                <div class="tool-item" style="background: var(--vscode-activityBar-background); border-left: 4px solid var(--vscode-button-background); border-radius: 4px; overflow: hidden;">
                    <div class="tool-header" onclick="toggleTool(event, ${index})" style="display: flex; align-items: center; gap: 12px; padding: 16px; cursor: pointer; user-select: none; transition: background-color 0.15s ease;" onmouseenter="this.style.backgroundColor='rgba(42, 82, 152, 0.04)'" onmouseleave="this.style.backgroundColor='transparent'">
                        <div style="flex: 1; display: flex; align-items: center; gap: 10px;">
                            <h3 style="font-size: 16px; font-weight: 600; color: var(--vscode-activityBar-inactiveForeground); margin: 0;">${escapeHtml(tool.name)}</h3>
                            <svg class="expand-icon" width="12" height="12" viewBox="0 0 12 12" style="transition: transform 0.2s ease; opacity: 0.6; flex-shrink: 0; transform: rotate(0deg);">
                                <path d="M2 4 L6 8 L10 4" fill="none" stroke="var(--vscode-activityBar-inactiveForeground)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </div>
                        ${headerAction}
                    </div>
                    <div class="tool-description" style="max-height: 0; overflow: hidden; transition: max-height 0.3s ease, padding 0.3s ease; padding: 0 16px;">
                        <p style="font-size: 13px; color: var(--vscode-activityBar-inactiveForeground); margin: 0; line-height: 1.5; padding-bottom: 16px; border-top: 1px solid #e0e0e0; padding-top: 12px;">${escapeHtml(tool.description || 'No description available')}</p>
                    </div>
                    ${paramsHtml}
                </div>
            `;
