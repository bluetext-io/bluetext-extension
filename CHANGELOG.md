# Change Log

All notable changes to the "bluetext-setup" extension will be documented in this file.

## [1.0.0] - 2025-03-11

### Added
- Initial release of Bluetext Setup Assistant
- Interactive setup wizard with step-by-step guidance
- Automatic polytope.yml file creation
- Cline MCP configuration support
- Claude Code MCP configuration support
- Git repository initialization
- MCP server startup integration
- Cross-platform support (macOS, Windows, Linux)
- VSCode and VSCodium compatibility
- Configurable MCP port setting
- Individual commands for each setup step:
  - Create polytope.yml
  - Configure Cline
  - Configure Claude Code
  - Initialize Git
  - Start MCP Server
- Comprehensive documentation and troubleshooting guide

### Features
- **Setup Wizard**: Interactive web-based wizard guiding users through the entire setup process
- **File Configuration**: Automated creation and configuration of necessary files
- **MCP Integration**: Seamless integration with Model Context Protocol servers
- **Coding Agent Support**: Built-in support for Cline and Claude Code
- **User-Friendly**: Clear error messages and helpful prompts throughout the process

### Documentation
- Complete README with usage instructions
- Troubleshooting guide
- Links to relevant documentation
- Configuration options reference
